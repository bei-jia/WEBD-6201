...
import * as validateForm from './validate.js';
...
if ($("#loginSubmit")) {
    $("#loginSubmit").click(function (e) {
        ...
        // If the user is already logged in, the user name is no longer appended to the nav bar
        if($('#user-Name').length>0) {console.log('already login');}

        // If the user not login, then add the user name between contact and login link
        else{
            let userName=$('#log-username').val();
        let loginPage=$('#lognin');

        // insert the input user name between contact and lonin
        let userLink=$('<li class="nav-item" id="user-Name"><a class="nav-link" href="login.html">'+userName+'</a></li>').insertBefore(loginPage);
        }
...
// if the register submit button is on the page
if ($("#btnRegister")) {
    // add a click event 
    $("#btnRegister").click(function (e) {
        // when the user clicks on the Register button prevent the default form behaviour
        e.preventDefault();
        ...
        const instance_user = new userClass.User(
            ...
            console.log(`User details:${instance_user.displayUser()}`)

        // Validation part. An empty div is returned if it passes validation, otherwise a div with a hint is returned    
        // validate first name
        $("#inputFirstName").next(".errorMessage").html(validateForm.validateFirst(instance_user.firstName));
        
        // validate last name
        $("#inputLastName").next(".errorMessage").html(validateForm.validateLast(instance_user.lastName));
        
        // validate  username
        $("#inputEmail").next(".errorMessage").html(validateForm.validateUsername(instance_user.email));

        // validate confirm password
        let error=validateForm.validatePassword(instance_user.password,$("#inputPassword2").val());
        
        $("#inputPassword").next(".errorMessage").html(error);
        $("#inputPassword2").next(".errorMessage").html(error);
